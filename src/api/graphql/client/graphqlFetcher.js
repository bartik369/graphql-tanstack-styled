export const graphqlFetcher = (query, variables) => {
    return async () => {
        ...
        if (fileEntries.length > 0) {
            ...
            const variablesWithoutFiles = Object.fromEntries(entries.map(([key, value]) => [key, value instanceof File ? null : value]));
            formData.append('operations', JSON.stringify({ query, variables: variablesWithoutFiles }));
            const map = Object.fromEntries(fileEntries.map(([name,], i) => [String(i), [`variables.${name}`]]));
